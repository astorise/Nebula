...
pub trait TeacherModel {
    fn generate_curriculum(&self, subject: &str, count: usize, schema: &serde_json::Value) -> Result<Vec<CurriculumTask>>;
}
...
pub fn generate_and_inject(
    ...
    for task in tasks {
        let event = AgentInferenceEvent {
            ...
            prompt: format!("{}\n\n{}\n\nConstraints:\n{}", task.title, task.description, task.constraints.join("\n")),
...
mod tests {
    ...
    impl TeacherModel for Teacher {
        fn generate_curriculum(&self, _subject: &str, count: usize, _schema: &serde_json::Value) -> Result<Vec<CurriculumTask>> {
            Ok((0..count).map(|idx| CurriculumTask {
    ...
    fn injects_correlation_header() {
        ...
        let events = generate_and_inject(&Teacher, &mut bus, CurriculumRequest {
