...
function render(): void {
  ...
  document.getElementById("saveConstitution")?.addEventListener("click", () => {
    const rules = ((document.getElementById("constitutionInput") as HTMLTextAreaElement | null)?.value ?? "")
      .split("\n")
      .map((line) => line.trim())
  ...
  document.getElementById("tenantSelect")?.addEventListener("change", (event) => {
    vscode.postMessage({
      ...
      payload: { tenantId: (event.target as HTMLSelectElement).value }
    });
  });
  ...
  document.getElementById("replayRatio")?.addEventListener("change", (event) => {
    vscode.postMessage({
      ...
      payload: { ratio: Number.parseFloat((event.target as HTMLInputElement).value) }
    });
  });

  document.querySelectorAll<HTMLButtonElement>("[data-preference]").forEach((button) => {
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-purge-tenant]").forEach((button) => {
    button.addEventListener("click", () => {
      vscode.postMessage({
        type: "COMMAND",
        action: "tenant.purge",
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-golden-pin]").forEach((button) => {
    button.addEventListener("click", () => {
      vscode.postMessage({
        type: "COMMAND",
        action: "golden.pin",
  ...
  document.querySelectorAll<HTMLButtonElement>("[data-foundry-approve]").forEach((button) => {
    button.addEventListener("click", () => {
      vscode.postMessage({
        type: "COMMAND",
...
function tenantOptions(): string {
  const tenants = state.tenants.tenants.length === 0 ? [{ tenantId: state.tenants.activeTenantId, rows: 0, quota: 50_000 }] : state.tenants.tenants;
  return tenants
    .map(
      (tenant) =>
        `<option value="${escapeHtml(tenant.tenantId)}" ${tenant.tenantId === state.tenants.activeTenantId ? "selected" : ""}>${escapeHtml(tenant.tenantId)}</option>`
    )
    .join("");
}

function tenantRows(): string {
...
function foundryRows(): string {
  ...
  return state.foundry.pendingTools
    .map(
      (tool) => `
        <div class="peer">
